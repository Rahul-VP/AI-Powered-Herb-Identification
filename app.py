...
def plot_top5_predictions(classes, probs):
    """Create a horizontal bar chart for top 5 predictions with 50% threshold."""
    arr = np.asarray(probs, dtype=float).ravel()  # ensure numeric array
    top1_prob = float(arr[0]) if arr.size else 0.0
    top1_class = classes[0] if classes else "—"
    title = f"Prediction: {top1_class} ({top1_prob*100:.1f}%)"
    if top1_prob < MIN_CONF:
        title = "Prediction: (Not identified)"

    fig = go.Figure(go.Bar(
        x=arr * 100,
        y=classes,
        orientation='h',
        marker=dict(color=arr * 100, colorscale='Viridis', showscale=False),
        text=[f'{p*100:.2f}%' for p in arr],
        textposition='auto',
    ))
    fig.update_layout(
        title=title,
...
def main():
            ...
            with st.spinner("Analyzing image..." if not use_tta else "Running TTA analysis (this may take a moment)..."):
                try:
                    ...
                    pred_conf = top5_probs[0]
                     # NEW: thresholded display name
                    display_name = "(Not identified)" if float(pred_conf) < MIN_CONF else pred_class
                     # NEW: identified flag
                    is_identified = float(pred_conf) >= MIN_CONF
                    ...
                    <div style="background: linear-gradient(135deg, {conf_color} 0%, {conf_color}dd 100%); 
                        ...
                        <h1 style="color: white; margin: 0; font-size: 2.2rem; text-shadow: 2px 2px 4px rgba(0,0,0,0.2);">{conf_icon} {display_name}</h1>
    ...
    # ...existing code...
    # Medicinal Information Section (full width) - as a bonus feature
    if uploaded_file is not None and show_medicinal_info and 'pred_conf' in locals() and is_identified:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("""
        ...
        """, unsafe_allow_html=True)
        try:
            # Always show info; warn if confidence is low
